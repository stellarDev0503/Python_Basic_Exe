...
if inputFileName is None:  # exit on "Cancel"
    exit()
...
if pageStart is None:  # exit on "Cancel"
    exit()
...
while not pageStart.isdigit() or pageStart == \
        "0" or int(pageStart) > totalPages:
...
if pageEnd is None:  # exit on "Cancel"
    exit()
# check for possible problems and try again:
#    1) input page number isn't a (non-negative) digit
# or 2) input page number is 0
